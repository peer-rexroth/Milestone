...
| `verify_cost` | cost tracking: resource rates, rate × duration rollup (leaf, group, cost to date), the Resource rates dialog, the Cost/Cost to Date columns and their currency formatting, the task dialog's row |
| `verify_resources` | the Resources view: workload and cost per resource, overallocation detection and grouping into ranges, jumping to a task, the empty state |
