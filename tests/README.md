...
| `verify_alap_snlt` | true ALAP (pulled to the latest a successor allows, multiple successors, over-constrained, no successors) and enforced SNLT/FNLT (caps a forward push, still just flags with nothing pushing) |
| `verify_reschedule` | reschedule remaining work from a status date: complete/manual/future tasks untouched, overdue and in-progress tasks moved (never pulled earlier), cascading through dependencies in one sweep, the dialog and its Undo |
| `verify_schedule_menu` | Baseline / Reschedule / Working calendar / Scheduling precision split out of the plan menu into their own Schedule dropdown: the split itself, opening each dialog, mutual exclusivity with Data, click-outside and Escape |
| `verify_workhours_calendar` | minute-mode scheduling precision, the working-hours calendar layer: on/before/after a break, across a weekend/holiday, the O(1) bulk-day shortcut vs. a brute-force walk, lag/duration caps, `normalizeData()` cleaning, the MSPDI wiring |
| `verify_minute_schedule` | minute-mode scheduling precision, the engine: every link type with minute lags across a break/weekend/holiday, ALAP and SNLT with a time component, critical-path float in minutes, a time-of-day reschedule status moment, non-destructive day↔minute switching |
| `verify_minute_ui` | minute-mode scheduling precision, the UI: the precision toggle and working-hours/breaks editor in its own Schedule-menu dialog, the task dialog's time inputs, its own widened modal, and hour/minute duration entry, the grid's date+time display and adapted column widths, day mode reverting all of it |
| `verify_minute_baseline` | minute-mode scheduling precision, baselines: a baseline keeps its own time, day-mode/timeless baselines stay the plain 2-element array, minute-unit variance and its formatting, the grid/tooltip/task-dialog display including the live-as-you-type variance, a group's variance never switching to minutes |
| `verify_minute_files` | minute-mode scheduling precision, file round-tripping: Excel date+time cells and pre-formatted duration/variance text, a day-mode export staying byte-identical, MSPDI export's real per-task/per-baseline times and minute-precise duration, MSPDI import's mode-detection heuristic and recovery of real working hours from `<WorkingTimes>`, the "import calendar too" checkbox gating it |
| `verify_minute_gantt` | minute-mode scheduling precision, the chart: the Hours scale (only in a minute-mode plan) — bar geometry at 20px an hour, the day-over-hours header, shaded nights/lunch/days off, the today line, drag and resize in 15-minute steps (an Auto task lands on working time across the lunch break, a Manual one where dropped, successors follow, the Start-No-Earlier-Than pin carries its time), group rollups, baseline bars, the day scales staying whole-day, the day-mode fallback, and the Excel Gantt sheet's one-column-per-working-hour layout and its 400-column fallback |
| `verify_minute_inline` | minute-mode scheduling precision, the list: the typed `dd.mm.yyyy hh:mm` cell editor for Start/Finish/Actual (date-only and time-only, ISO, the calendar button, lunch-break snapping, a 17:00/12:00 finish staying exact, refusals, Manual tasks, milestones, started tasks), minute Duration entry/display, day mode unchanged |
| `verify_version_skew` | plans stamped by another build: a minute-mode plan an older build left behind is realigned once (on load and on a file merge), a newer build's plan is warned about, version-2 and day-mode plans are left alone |
