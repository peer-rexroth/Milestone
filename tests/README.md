...
| `verify_schedule_menu` | Baseline / Reschedule / Working calendar split out of the plan menu into their own Schedule dropdown: the split itself, opening each dialog, mutual exclusivity with Data, click-outside and Escape |
...
| `verify_minute_ui` | minute-mode scheduling precision, the UI: the precision toggle and working-hours/breaks editor, the task dialog's time inputs, its own widened modal, and hour/minute duration entry, the grid's date+time display and adapted column widths, day mode reverting all of it |
