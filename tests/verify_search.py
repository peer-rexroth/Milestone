...
with sync_playwright() as p:
    ...
    check("a result shows the ID, the name, the WBS code, its groups (Website relaunch › Design) and its dates", "Wireframes" in row and "1.1.2" in row and "Website relaunch › Design" in row and "07.09.2026" in row and "#" in row, row)
    ...
    check("with a single match there is no 'show only' hint", ev("() => getComputedStyle(document.querySelector('#searchFoot [data-shift]')).visibility") == "hidden")
    pg.keyboard.press("Escape")
