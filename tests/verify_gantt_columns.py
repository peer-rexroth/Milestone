...
with sync_playwright() as p:
    ...
    check("garbage in the Gantt set is ignored: unknown columns dropped, Task Name can never be hidden, columns this version knows but the save doesn't are appended and follow the Gantt default (the default ones shown) — Cost/Cost to Date land right after Resource (COLS_REV_MOVE: this save has no revision, so it's caught up like any older install)", "name" in g3 and "bogus" not in o3 and g3 == ["resource", "start", "name", "wbs", "duration", "baselineStart", "baselineFinish", "durationVariance"] and o3[:5] == ["resource", "cost", "costToDate", "start", "name"] and set(o3) == set(pg3.evaluate("() => DEFAULT_COL_ORDER")), (g3, o3[:6]))
