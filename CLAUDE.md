...
**Sibling projects**: [Pulse](../Pulse/CLAUDE.md) (a programme dashboard) and mytasks (a task manager) are this same author's other single-file apps — dbPlanner's PWA setup and data-durability mechanisms are ported from Pulse (see its own CLAUDE.md for the fuller narrative history behind those patterns; dbPlanner's own version below is the same design, freshly built rather than copy-pasted). dbPlanner also initially ported Pulse's switchable color-scheme system but later dropped it for a single fixed palette — see "Fixed color palette" below.
...
**Task Mode** (`task.taskMode`: `'auto'` | `'manual'`, default `'auto'`) is MS Project's own Manually Scheduled vs Auto Scheduled distinction, and behaves exactly like it: a `'manual'` task is skipped by both `cascadeSchedule()` and `applyConstraints()`'s self-shift step (same `continue`/guard shape as the cyclic-task skip above) — its dates are never rewritten by a predecessor's push, but it still keeps whatever dates it has for *its own* successors' `constraintStart()` to read, so downstream tasks schedule correctly off it either way. `toggleTaskMode(id)` flips the mode and, when switching back to `'auto'`, immediately calls `applyConstraints(id)` to snap the task into cascade-compliant position — the same "recalculate on re-link" behavior real MS Project shows. The grid shows this as its own column (`.task-mode-cell`, a hand icon for manual/amber, gears for auto/dim) positioned right before Task Name, matching MS Project's own column order; summary tasks show a fixed, non-interactive gears icon since their dates are computed rollups (see `effectiveDates()`), not stored — there's nothing to toggle.
...
Only leaf tasks get start/finish/duration/predecessors inline editing (a summary task's own dates are computed rollups — see `effectiveDates()` above — so editing them inline has nowhere to write); milestones additionally skip finish/duration (forced equal to start by definition). All of this mirrors the task modal's own rules, just enforced at the point where the click would start an edit rather than after the fact.
